/**
 * 
 */
package com.example.musicname;

import android.app.Notification;
import android.app.NotificationManager;
import android.widget.RemoteViews;

/**
 * Title: PlayerMsg.java
 * Description: 
 * Copyright:Copyright(c)2015
 * Company: 四川久聚科技股份有限公司
 * CreateTime:2015-8-28 下午4:01:50 
 * @author liuwenxing
 * @version 1.0
 */
public class ConstUtil {

	    //MusicBox接收器所能响应的Action
	    public static final String MUSICBOX_ACTION="com.example.musicname.MUSICBOX_ACTION";
	    //MusicService接收器所能响应的Action
	    public static final String MUSICSERVICE_ACTION="com.example.musicname.MUSICSERVICE_ACTION";
	  //PlayFragment接收器所能响应的Action
	    public static final String PLAYFRAMGMENT_ACTION="com.example.musicname.PLAYFRAMGMENT_ACTION";
	    //初始化flag
	    public static final int STATE_NON=0x122;
	    //播放的flag
	    public static final int STATE_PLAY=0x123;
	    //暂停的flag
	    public static final int STATE_PAUSE=0x124;
	    //停止放的flag
	    public static final int STATE_STOP=0x125;
	    //播放上一首的flag
	    public static final int STATE_PREVIOUS=0x126;
	    //播放下一首的flag
	    public static final int STATE_NEXT=0x127;
	    //菜单关于选项的itemId
	    public static final int MENU_ABOUT=0x200;
	    //菜单退出选的项的itemId
	    public static final int MENU_EXIT=0x201;
	    public static int musicNum = 0;
	    public final static String ACTION_BTN = "com.example.music";
	    public static String play = ""; 
	    public static String musicName = "";
	    public static RemoteViews re;
	    public static Notification no;
	    public static NotificationManager nf;
	     
	    public ConstUtil() {
	        // TODO Auto-generated constructor stub
	    }
	 
	
}
